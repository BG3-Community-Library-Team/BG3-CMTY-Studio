<script lang="ts">
  ...
  import { SECTION_HINTS } from "../lib/data/sectionHints.js";
  ...
  let autoLayout = $derived(schema ? autoLayoutFromSchema(schema) : null);
  let hints = $derived(SECTION_HINTS[section]);
...
<div class="schema-form">
  ...
  {:else}
    ...
    {#if dataAttrs.length > 0}
      {@const mainAttrs = dataAttrs.filter(a => a.frequency >= 0.8)}
      {@const optionalAttrs = dataAttrs.filter(a => a.frequency >= 0.3 && a.frequency < 0.8)}
      {@const rareAttrs = dataAttrs.filter(a => a.frequency < 0.3)}

      {#if mainAttrs.length > 0}
        <FormSectionCard title="Fields" open>
          <fieldset class="space-y-1.5">
            {#each mainAttrs as attr (attr.name)}
      ...
      {#if optionalAttrs.length > 0}
        <FormSectionCard title="Optional Fields" open>
          <fieldset class="space-y-1.5">
            {#each optionalAttrs as attr (attr.name)}
              {@const badge = typeBadge(attr)}
              <div class="flex items-center gap-2">
                <span class="text-xs text-[var(--th-text-400)] w-36 shrink-0 truncate" title="{attr.name} — {attr.attr_type} ({Math.round(attr.frequency * 100)}%)">
                  {attr.name}
                  <span class="text-[10px] text-[var(--th-text-600)]"> {m.common_optional()}</span>
                </span>
                <div class="relative flex-1">
                  <input
                    type={inputType(attr)}
      ...
      {#if rareAttrs.length > 0}
        <FormSectionCard title="Rare Fields" open={false}>
          <fieldset class="space-y-1.5">
            {#each rareAttrs as attr (attr.name)}
              {@const badge = typeBadge(attr)}
    ...
    {#if guidAttrs.length > 0}
      <FormSectionCard title="References" open>
        <fieldset class="space-y-1.5">
    ...
    {#if boolAttrs.length > 0}
      <FormSectionCard title="Booleans" open>
        <fieldset class="mb-0">
    ...
    {#if schema.children.length > 0}
      <FormSectionCard title="Child Groups" open>
